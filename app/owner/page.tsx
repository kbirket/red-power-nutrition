    ...
    <section className={`panel ${open ? "editing" : "locked"}`} key={x.id}>
      ...
      <div className="special-drinks">
        <div className="option-title">
          ...
          <div className="special-add-actions"><button onClick={() => addSpecialItem(x.id)}>+ Create new drink</button><select defaultValue="" onChange={(e) => { addExistingSpecialItem(x.id,e.target.value); e.currentTarget.value=""; }}><option value="" disabled>+ Add from regular menu</option>{items.filter((item)=>!specialItems.some((d)=>d.special_menu_id===x.id&&d.menu_item_id===item.id)).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></div>
        </div>
...
{tab==="orders"&&<section className="panel"><h2>Orders</h2>{orders.map(x=><div className="rank" key={x.id}><span>{new Date(x.created_at).toLocaleString()} · {x.fulfillment||"pickup"} · {x.status}</span><b>{money(x.total)}</b></div>)}</section>}</div><style jsx>{`.locked{opacity:.62;background:rgba(255,255,255,.45)}.locked input,.locked select{cursor:not-allowed}.editing{opacity:1}.option-row,.special-drink{display:flex;gap:10px;align-items:center;padding:10px;border-radius:12px;margin:7px 0}.option-row>input{flex:1}.special-drink input{flex:1;min-width:120px}.option-row input,.option-row select,select{min-width:0}.menu-card{padding:12px 0;border-top:1px solid rgba(0,0,0,.08)}.flavor-section,.special-drinks{margin:4px 0 0 22px;padding:10px 12px;border-left:3px solid rgba(190,45,35,.35)}.flavor-section h4,.special-drinks h3{margin:0}.special-drinks{margin-left:0;border-left:0;border-top:1px solid rgba(0,0,0,.08);padding:18px 0 0}.special-name{font-weight:700;font-size:18px;margin:0 10px 10px 0}.date-row{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-top:10px}.date-row label{display:flex;gap:6px;align-items:center}.add-bottom{display:block;margin:16px auto 2px;min-width:220px}.empty-drinks{padding:10px 0}.muted{opacity:.65}@media(max-width:700px){.option-row,.special-drink{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:14px}.option-row>input,.special-drink input:first-child,.special-drink input:nth-child(2){grid-column:1/-1;width:100%;min-height:44px}.money-input{grid-column:1/2;min-height:44px}.available{grid-column:2/3;align-self:center}.option-row button,.special-drink button{min-height:44px;width:100%}.add-bottom{width:100%;min-height:48px;margin-top:18px}.panel{padding-left:12px;padding-right:12px}.section-head{gap:12px}.section-head .red{white-space:nowrap}.date-row label{width:100%;justify-content:space-between}.date-row label input[type=date]{max-width:62%}.special-drinks .option-title{align-items:flex-start}.special-drinks .option-title button{flex:none}.special-add-actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.special-add-actions select{min-height:40px;padding:0 10px;border-radius:10px;border:1px solid rgba(0,0,0,.15);background:white}@media(max-width:700px){.special-add-actions{width:100%}.special-add-actions button,.special-add-actions select{width:100%}}}`}</style></main>}

...
export default function OrderPage() {
    ...
    Promise.all([
      ...
      if (m.data?.length) {
        const items: Drink[] = m.data.map((i: any) => ({
          id: i.id, name: i.name, price: Number(i.price), description: i.description,
          category: i.menu_categories?.name === "Protein Shakes" ? "shake" : "tea",
        }));
    ...
    try {
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          schoolScheduleId,
          items: cart.map((item) => ({
            menuItemId: item.id.startsWith("tea-") || item.id.startsWith("shake-") ? null : item.id,
            name: item.name,
            price: item.price,
            quantity: 1,
            addons: item.addons,
      ...
      setSuccess("Order received! Red Power Nutrition will take it from here.");
      setCart([]); setName(""); setPhone(""); setTeacher(""); setNotes("");
    } catch (err) {
      console.error(err);
      setError(err instanceof Error ? err.message : "Something went wrong sending your order. Please try again.");
    } finally { setSubmitting(false); }
  };

  const renderDrinks = (drinks: Drink[]) => drinks.map((drink) => (
    <button className="drink-card" key={drink.id || drink.name} onClick={() => { setSelected(drink); setSelectedAddons([]); }}>
      <strong>{drink.name}</strong><span>{drink.description || (drink.category === "tea" ? "Loaded tea" : "Protein shake")}</span><b>{money(drink.price)}</b><small>Customize +</small>
    </button>
  ));
  ...
  return <main className="order-page">
    ...
    <div className="order-layout"><div className="menu-column">
      <section><div className="section-title"><h2>🧋 Loaded Teas</h2><span>{teas.length} flavors</span></div><div className="drink-grid">{renderDrinks(teas)}</div></section>
      <section><div className="section-title"><h2>🥤 Protein Shakes</h2><span>{shakes.length} flavors</span></div><div className="drink-grid">{renderDrinks(shakes)}</div></section>
    </div>
    <aside className="checkout panel"><div className="checkout-head"><div><p className="eyebrow">YOUR ORDER</p><h2>{cart.length ? `${cart.length} drink${cart.length === 1 ? "" : "s"}` : "Your cart is empty"}</h2></div><strong>{money(subtotal)}</strong></div>
      {cart.map((item) => <div className="cart-item" key={item.cartId}><div><strong>{item.name}</strong>{item.addons.length > 0 && <small>{item.addons.map((addon) => addon.name).join(" • ")}</small>}</div><div><b>{money(item.price + item.addons.reduce((sum, addon) => sum + addon.price, 0))}</b><button onClick={() => setCart((current) => current.filter((x) => x.cartId !== item.cartId))}>×</button></div></div>)}
      ...
      <button className="submit-order" disabled={submitting || !cart.length} onClick={submitOrder}>{submitting ? "Sending order..." : `Place order • ${money(subtotal)}`}</button><small className="payment-note">Payment is coming soon. This version sends the order directly to Red Power Nutrition.</small>
    </aside></div>
    {selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><div className="drink-modal" onClick={(e) => e.stopPropagation()}><button className="close" onClick={() => setSelected(null)}>×</button><p className="eyebrow">CUSTOMIZE</p><h2>{selected.name}</h2><p className="modal-price">Base drink {money(selected.price)}</p><h3>Add a boost</h3><div className="addon-list">{addons.map((addon) => { const isSelected = selectedAddons.some((x) => x.id === addon.id); return <button className={isSelected ? "addon active" : "addon"} key={addon.id || addon.name} onClick={() => toggleAddon(addon)}><span>{isSelected ? "✓" : "+"} {addon.name}</span><b>+{money(addon.price)}</b></button>; })}</div><button className="submit-order" onClick={addToCart}>Add to cart • {money(modalTotal)}</button></div></div>}
  </main>;
}
